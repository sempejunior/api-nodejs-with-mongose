...
router.post('/', async (req, res) => {
    ...
    if (!email || !password) {
        return res.status(400).send({ error: 'Dados insufientes' });
    }
    try {
...
router.post('/auth', async (req, res) => {
    ...
    try {
        ...
        if (!user) res.status(401).send({ error: 'Erro na consulta de usuários' });

        const samePassword = await bcrypt.compare(password, user.password);

        if (!samePassword) return res.status(401).send({ error: 'Erro ao autenticar o usuário.' });
